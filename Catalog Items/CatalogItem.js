function CatalogItemUtils() {
	...
	if(!ga.next())
		return gs.print("Attchment is not defined");
	...
	this.createDefinitions = function(settings) {
		...
		try {
			...
			gs.print(output_result);

		} catch(ex) {
			output_result += "\n\n[Exeption] createDefinitions: " + ex;
		}
	};
	...
	this._parseFormVariables = function() {
		...
		try {
			...
			while(ps.next()) {
				...
				// 	gs.print("Question: " + r["Question"]);
				// 	gs.print("Text: " + c.text);
				// 	gs.print("Value: " + c.value);
				// 	gs.print("Order: " + c.order);
				// 	gs.print("\n");
				// });
			...
			output_result += "\n\n[Exeption] _parseFormVariables: " + ex;
		}
	};
	...
	this._defineVariableOrder = function() {
		...
		try {
			...
			output_result += "\n\n[Exeption] _defineVariableOrder: " + ex;
		}
	};
		...
		try {
			...
			for(var c = 0; c < criteries.length; c++) {
				...
				if(grMTOM.insert()) {
					...
					output_result += "\n\n[Failed] _createUserCriteria: " + short_description;
			}

		} catch(ex) {
			output_result += "\n\n[Exeption] _createUserCriteria: " + ex;
		}
	};
	...
	this._createTaskDefinitions = function() {
		...
		try {
				...
				switch(type){
					case "Fulfillment":
						...
						if(taskDefinitionGR.insert()) {
							...
							output_result += "\n\n[Failed] _createTaskDefinitions: " + short_description;
			...
			output_result += "\n\n[Exeption] _createTaskDefinitions: " + ex;
		}
	...
	this._getGroupIDByName = function(name) {
		try {
			...
			output_result += "\n\n[Exeption] _getGroupIDByName: " + ex;
		}
	}
}

var cat_item_id = '9b94847d1ba6b7c0cd6298efbd4bcb87';
